    ...
    try:
        ...
        while True:
            ...
            for file in response.get('files', []):
                ...
                print('Found file:', (file.get('name'), file.get('id'), file.get('trashed'),file.get('parents')))

            page_token = response.get('nextPageToken', None)
            if page_token is None:
                break
        exit()
        # now create a file without recreating the service
        ...
        quit()
    # trashed = false
    #         parent = '1OEAdTnhQoAKpyzFqYdM1ztr42vHAqbk5', "name='sub folder'",

    # results = drive_service.files().list(q=
    #                                          "mimeType = 'application/vnd.google-apps.folder'",
    #                                pageSize=10, fields="nextPageToken, files(id, name)",
    #                                supportsAllDrives=True, driveId='driveId', includeItemsFromAllDrives=True,
    #                                corpora='drive').execute()
    #     print('File ID, name: ', file.get('id'))

    # #from apiclient import errors
    # # ...
    #
    # # def retrieve_all_files(drive_service):
