...
if choice == "Exit":
    ...
    # TODO: Need to upload the Admin file as well
    if False:
        if False:
    ...
    try:
        ...
        delList = getFolderId(folderName, REPORT_FOLDER_ID)
        for folderId in delList:
    ...
    for org in orgs:
        ...
        df_pt = pd.pivot_table(xlo, columns=[reportVar], index=['team_name'], values=['addresses_count'], margins=True, dropna=True,
                               aggfunc=np.sum)
        df_pt.to_excel(writer, sheet_name='Team Summs', startrow=6) # summary sheet of al teams combined
        ...
        df_pt = pd.pivot_table(xlo, columns=[reportVar], index=['team_name', 'writer_name'], values=['addresses_count'], margins=True, dropna=True,
                               aggfunc=np.sum)
        df_pt.to_excel(writer, sheet_name='Teams wWrtrs', startrow=6) # summary sheet of al teams combined
        ...
        df_pt = pd.pivot_table(xlo, columns=[reportVar], index=['team_name', 'writer_name'], values=['addresses_count'], margins=True, dropna=True,
                               aggfunc='count')
        df_pt.to_excel(writer, sheet_name='Teams wCounts', startrow=6) # summary sheet of al teams combined
        ...
        for sh in wb.worksheets:
         #   room = os.path.pathsplitext(InputFile)[0].split("/")[-1].split("_")[0]
            ...
            if sh.title not in ['Campaigns','Team Summs','Teams wWrtrs','Teams wCounts']:
